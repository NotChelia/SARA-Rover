...
public:
    ReedsSheppPlanner(double turning_radius = 1.0) 
    ...
    Path plan(const Pose2D& start, const Pose2D& goal) {
        ...
        compute_CSC_paths(x, y, phi, paths);
        compute_CCC_paths(x, y, phi, paths);
...
private:
    ...
    void compute_CSC_paths(double x, double y, double phi, std::vector<Path>& paths) {
        {
            Path path;
            double r = turning_radius_;
    ...
    void compute_CCC_paths(double x, double y, double phi, std::vector<Path>& paths) {
        {
            Path path;
            double r = turning_radius_;
            double u = x - r * std::sin(phi);
            double v = y + r * std::cos(phi);
            double d = std::sqrt(u*u + v*v);
            
            if (d <= 4 * r) {
                double alpha = std::acos(d / (4 * r));
                double t = std::atan2(v, u);
                double beta = t - alpha;
                
                path.add_segment(PathSegmentType::LEFT, r * beta);
                path.add_segment(PathSegmentType::RIGHT, r * 2 * alpha);
                path.add_segment(PathSegmentType::LEFT, r * (phi - beta + 2 * alpha));
        ...
        }
        {
            ...
            if (d <= 4 * r) {
                ...
                path.add_segment(PathSegmentType::RIGHT, r * beta);
                path.add_segment(PathSegmentType::LEFT, r * 2 * alpha);
                path.add_segment(PathSegmentType::RIGHT, r * (beta - phi + 2 * alpha));
